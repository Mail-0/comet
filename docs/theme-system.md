...
Keiki themes are complete, source-neutral `ThemeVariant` values owned by the
`keiki-theme` crate. Runtime components consume only Keiki semantic roles. VS
...
- Every variant records a recommended `SurfaceTreatment`: Keiki recommends
...
instead of fixed Keiki greys. Its window tint becomes denser when necessary to
...
still allowing the independent Keiki surface preference to force frost.

After source mapping, a deterministic hardening pass checks Keiki's shared
...
related source role when the preferred one cannot carry the Keiki role, and
...
The `keiki-theme-import` development tool exposes the same single-file adapter
for built-in curation:

Example:

```bash
cargo run -p keiki-theme --bin keiki-theme-import -- \
  --input /path/to/theme.json \
  --output /tmp/theme.keiki.json \
...
source. Keiki does not fetch or execute VSIX packages at runtime; custom
