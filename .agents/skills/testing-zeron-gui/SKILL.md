...
- `ZERON_FORCE_GATE=signin` — always render the sign-in gate card (the only place the
  "Sign in to Keiki" button lives). Note: with this set the gate stays up *even after a
  successful sign-in*, so you cannot observe the signed-in shell in the same run — use it only
  for gate-level tests (button states, error text, adversarial clicks).
...
## Two-stage launch: how to observe the *signed-in* shell

The "Sign in to Keiki" button only exists on the sign-in gate, and `ZERON_FORCE_GATE=signin` is
sticky (`shell.rs`: `self.debug_gate.clone().unwrap_or_else(|| state.gate())`), so the gate never
dismisses. Run two zeron processes sequentially inside ONE wrapper session:
...
- "Sign out of Keiki" must clear the `keiki-cloud` device (Settings → Devices), the `keiki-agent:`
  projects and the `keiki-conv:` chats while leaving the local device and "All projects" — check
  sidebar, project picker and Settings → Devices separately, they have failed independently before.
