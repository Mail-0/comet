...
//!
//! The set is retained from the upstream Comet presentation layer:
...
//!   are ports of upstream hand-drawn inline SVGs.
//! - The harness brand marks (`claude-mark`, `openai-mark`, `cursor-mark`) are
//!   upstream ports. gpui tints SVGs with the text color, so the
//!   Claude mark's brand orange is available through [`claude_brand`].
//!
...
icon_assets![
    ...
    // Mirrored variant (upstream window controls used `-scale-x-100`): the LEFT
...
/// The Claude mark's brand orange (`#D97757`) — retained even on the
/// monochrome surface.
