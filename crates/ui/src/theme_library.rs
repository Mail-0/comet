...
use gpui::{App, Global};
use keiki_theme::vscode::SourceCompilation;
use keiki_theme::{
...
fn reconcile_and_refresh(cx: &mut App) {
    ...
    for appearance_kind in [Appearance::Light, Appearance::Dark] {
        let model = if appearance_kind.is_light() {
            keiki_theme::Appearance::Light
        } else {
            keiki_theme::Appearance::Dark
        };
...
mod tests {
    ...
    fn failed_persistence_leaves_library_state_unchanged() {
        ...
        next.entries.push(CustomThemeEntry {
            ...
            source: keiki_theme::CustomThemeSource::ImportedSnapshot {
                imported_from: None,
            },
            family: keiki_theme::ThemeFamily {
                id: "unpersisted".into(),
                name: "Unpersisted".into(),
                variants: Vec::new(),
            },
            ...
            status: keiki_theme::CustomThemeStatus::Ready,
        });
