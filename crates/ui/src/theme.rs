...
//! lightness steps; the same scale keiki's Tailwind theme used) into gpui [`Hsla`].
...
use keiki_syntax::HighlightKind;
use keiki_theme::{
...
pub enum AccentColor {
    /// The exact upstream Keiki indigo.
    #[default]
...
impl AccentColor {
    ...
    fn tokens(self, appearance: Appearance) -> AccentTokens {
        ...
        let (primary, strong) = match (self, appearance) {
            (Self::Keiki, Appearance::Dark) => {
                (oklch(0.673, 0.182, 276.935), oklch(0.585, 0.233, 277.117))
            }
            (Self::Keiki, Appearance::Light) => {
...
fn model_appearance(appearance: keiki_theme::Appearance) -> Appearance {
    match appearance {
        keiki_theme::Appearance::Dark => Appearance::Dark,
        keiki_theme::Appearance::Light => Appearance::Light,
    }
}
...
/// because tests outside this module may also exercise these helpers.
...
impl Theme {
    ...
    /// Main-panel header height (keiki `h-11`) — in-card headers (changes pane).
    ...
    /// Reserved status strip under the content outlet (keiki `h-6`) — the
    ...
    /// fixed Keiki grey would erase that palette's identity.
    ...
    /// Increase tint coverage only as far as needed for Keiki's shared text
    ...
    /// theme, so forcing frost does not reintroduce Keiki's neutral hover.
    ...
    pub(crate) fn from_variant(
        ...
        let accent_color = match accent_selection {
            AccentSelection::ThemeDefault => AccentColor::Keiki,
...
mod tests {
    ...
    fn neutral_950_is_0a0a0a() {
        // oklch(0.145 0 0) is Tailwind neutral-950, keiki's app background.
    ...
    #[test]
    fn keiki_accent_is_the_exact_upstream_default() {
        let dark = Theme::dark();
    ...
    fn theme_recommendation_and_surface_override_resolve_independently() {
        ...
        let keiki = Theme::dark();
        assert_ne!(catppuccin.surface, keiki.surface);
        ...
        let opaque_keiki = Theme::for_selection(
            Appearance::Dark,
            "keiki-dark",
            AccentSelection::ThemeDefault,
            SurfacePreference::Opaque,
        );
        assert_eq!(opaque_keiki.surface_treatment, SurfaceTreatment::Opaque);
        assert_eq!(opaque_keiki.glass(), opaque_keiki.surface);
    }
