...
#   target/package/keiki-<version>-macos-<arch>-app.tar.gz   (app archive)
...
APP_TARBALL="$OUT_DIR/keiki-$VERSION-macos-$ARCH-app.tar.gz"

cd "$ROOT"
cargo build --release -p keiki

rm -rf "$APP" "$DMG" "$APP_TARBALL"
mkdir -p "$APP/Contents/MacOS" "$APP/Contents/Resources"
install -m 755 "$ROOT/target/release/keiki" "$APP/Contents/MacOS/keiki"
...
# applied ahead of time; dist/keiki.png stays the full-bleed shared artwork).
ICONSET="$OUT_DIR/keiki.iconset"
...
iconutil -c icns "$ICONSET" -o "$APP/Contents/Resources/keiki.icns"
rm -rf "$ICONSET"
...
if $NOTARIZE; then
  # Staple the bundle before tarring it so the standalone archive also passes
  # Gatekeeper offline.
  ZIP="$OUT_DIR/keiki-notarize.zip"
...
# The standalone app archive.
tar -czf "$APP_TARBALL" -C "$OUT_DIR" Keiki.app
...
dmgbuild.build_dmg(
    ...
    settings={
        ...
        "icon": os.path.join(app, "Contents/Resources/keiki.icns"),
