...
#   target/package/keiki-<version>-linux-<arch>.tar.gz
...
if [[ "$PROFILE" == "release" ]]; then
  ...
  BIN="$ROOT/target/release/keiki"
else
  cargo build -p keiki
  BIN="$ROOT/target/debug/keiki"
fi

rm -rf "$STAGE" "$TARBALL"
mkdir -p "$STAGE"
install -m 755 "$BIN" "$STAGE/keiki"
install -m 644 "$ROOT/dist/keiki.desktop" "$STAGE/keiki.desktop"
install -m 644 "$ROOT/dist/keiki.png" "$STAGE/keiki.png"
...
install -Dm755 "$HERE/keiki" "$HOME/.local/bin/keiki"
install -Dm644 "$HERE/keiki.desktop" "$HOME/.local/share/applications/keiki.desktop"
install -Dm644 "$HERE/keiki.png" "$HOME/.local/share/icons/hicolor/1024x1024/apps/keiki.png"
