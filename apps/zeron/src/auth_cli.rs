...
pub async fn login(config: EngineConfig) -> anyhow::Result<()> {
    ...
    match auth.state() {
        AuthState::SignedIn { user, org_id } => {
            ...
            println!(
                "Sync is ready. Start or restart Keiki to open the synced workspace; existing local sessions will stay local."
            );
        }
    ...
    InstanceLock::acquire(&config.data_dir).map_err(|err| {
        anyhow::anyhow!(
            "{err}\nCannot {verb} while an engine is running — stop it first \
             (`zeron daemon stop`, or quit the Keiki app), or use the running UI instead."
        )
    })
}
